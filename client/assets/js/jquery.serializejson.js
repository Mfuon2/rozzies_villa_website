...
(function (factory) {
    ...
    $.fn.serializeJSON = function (options) {
        ...
        let opts = f.setupOpts(options); // validate options and apply defaults
        let typeFunctions = $.extend({}, opts.defaultTypes, opts.customTypes);

        // Make a list with {name, value, el} for each input element
        let serializedArray = f.serializeArray($form, opts);
        ...
        $.each(serializedArray, function (_i, obj) {
            ...
            let type = $(obj.el).attr("data-value-type");

            if (!type && !opts.disableColonTypes) { // try getting the type from the input name
                let p = f.splitType(obj.name); // "foo:string" => ["foo", "string"]
                nameSansType = p[0];
                type = p[1];
            }
            if (type === "skip") {
            ...
            let typedValue = f.applyTypeFunc(obj.name, obj.value, type, obj.el, typeFunctions); // Parse type as string, number, etc.

            if (!typedValue && f.shouldSkipFalsy(obj.name, nameSansType, type, obj.el, opts)) {
                return; // ignore falsy inputs if specified in the options
            }

            let keys = f.splitInputNameIntoKeysArray(nameSansType);
    ...
    $.serializeJSON = {
        ...
        serializeArray: function($form, opts) {
            ...
            return $form.map(function() {
                let elements = $.prop(this, "elements"); // handle propHook "elements" to filter or add form elements
                ...
                let $el = $(this);
                let val = $el.val();
                let type = this.type; // "input", "select", "textarea", "checkbox", etc.
        ...
        getCheckboxUncheckedValue: function($el, opts) {
            let val = $el.attr("data-unchecked-value");
            if (val == null) {
        ...
        shouldSkipFalsy: function(name, nameSansType, type, el, opts) {
            let skipFromDataAttr = $(el).attr("data-skip-falsy");
    ...
    let isObject =          function(obj) { return obj === Object(obj); }; // true for Objects and Arrays
    let isUndefined =       function(obj) { return obj === void 0; }; // safe check for undefined values
    let isValidArrayIndex = function(val) { return /^[0-9]+$/.test(String(val)); }; // 1,2,3,4 ... are valid array indexes
    let isArray =           Array.isArray || function(obj) { return Object.prototype.toString.call(obj) === "[object Array]"; };
}));
