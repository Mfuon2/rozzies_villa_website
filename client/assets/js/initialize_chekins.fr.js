...
let checkout_config = {
    ...
    onSelect: instance => {
        ...
        if (x.getDate() < y.getDate()) {
            lock_button = true
            Swal.fire({
                title: 'La date de départ doit être supérieure à la date d\'arrivée ',
                showClass: {
...
const save_locally = () => {
    ...
    if (checkinDate === "" || checkinDate === undefined) {
        showEmptyDialog("La date d'arrivée est requise")
        return;
    }
    if (checkOutDate === "" || checkOutDate === undefined) {
        showEmptyDialog("La date de départ est requise")
        return;
    }
    if (!lock_button) {
        ...
        return window.location.pathname = "fr/check.html";
    } else {
        Swal.fire({
            title: 'La date de départ doit être supérieure à la date d\'arrivée',
            showClass: {
...
(async function() {
    await updateDatesFun().then(() => {
        ...
        if (window.location.href.indexOf('status') !== -1) {
            ...
            Toast.fire({
                icon: 'success',
                title: 'Votre réservation a réussi. Merci d\'avoir choisi notre villa.'
            })
