...
const createBooking = async() => {
    ...
    Swal.fire({
        title: 'Confirmar',
        text: "Los siguientes datos se utilizarán para la reserva, ¿proceder al pago?",
        icon: 'success',
...
const makePayment = () => {
    ...
    if (phoneNumber.length < 10) {
        errors.push('Se requiere el número de teléfono')
    }
    if (email.length < 10 && !validEmail(email)) {
        errors.push('El correo electrónico es obligatorio o no es válido')
    }

    if (identityId.length < 7) {
        errors.push('La identidad es requerida es requerida o inválida')
    }
    if (errors.length) {
        warning.show()
        errors.forEach(r => {
